...
@Mod.EventBusSubscriber(modid = WaMod.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
public final class WaKeyBindingsForge {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (grounded) {
            ...
            int maxExtra = WaAirJumpClientState.getExtraJumps(playerId);
            boolean infinite = maxExtra == WaAirJumpSettings.INFINITE_EXTRA_JUMPS;
            ...
            if (nearWall && !walling) {
                ...
                if (dir.getAxis().isHorizontal()) {
                    if (dir.getAxis() == Direction.Axis.X) {
                        ...
                        WaNetwork.sendWall(true, true, value, player.getY());
                        synced = true;
                    } else {
                        ...
                        WaNetwork.sendWall(true, false, value, player.getY());
                        synced = true;
                    }
                }
                if (!synced) {
    ...
    private static boolean isSolid(Level level, BlockPos pos) {
        ...
        if (com.hhdxcz.wa.paper.WaWallBlacklist.isBlockedClient(level, pos)) {
            return false;
        }
